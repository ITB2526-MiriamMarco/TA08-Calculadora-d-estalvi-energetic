/**
 * ITB INFRASTRUCTURE ANALYZER - FINAL PRODUCTION VERSION
 * Features: Dynamic Year, Toggle Policies, Calendar-Aware Logic, Financial Summary.
 */

// Captura dinámica del año actual
const currentSystemYear = new Date().getFullYear();

const INFRA_DATA = {
    electricity: { variationRate: 0.2281 }, // 22.81% increase forecast
    water: {
        fixedDailyPerPax: 133,
        pricePerL: 0.0021, // ~2.10€/m3
        maintenanceLitersDay: 500 // Consumption when school is closed
    },
    costs: { cleaning: 175, supplies: 91.25 },
    energyPriceKwh: 0.24 // Average price in €
};
...
// Base de datos de políticas técnicas
const TECH_POLICIES = [
...
function runCalculations() {
    ...
    const schoolDays = 175; // Días lectivos fijos
    const idleDays = (selectedMode === 365) ? 190 : 0; // Días de cierre solo si se elige año completo

    // 1. CÁLCULO ENERGÍA (Días activos vs Vacaciones)
    ...
    // 4. APLICACIÓN DE AHORROS (Toggle)
    ...
    // 5. DEFINICIÓN DE MÉTRICAS (8 TARJETAS)
    const metrics = [
        ...
        { title: `${currentSystemYear + 1} Forecast`, val: currentEnergy * (1 + INFRA_DATA.electricity.variationRate), goal: (baseEnergy * 0.70) * (1.2281), unit: "kWh", icon: "📈" }
    ];
        ...
        grid.innerHTML += `
            <div class="card">
                ...
                <div class="data-container">
                    <div class="current-row"><span class="label">Actual:</span><span class="data">${Math.round(m.val).toLocaleString()}</span><span class="unit">${m.unit}</span></div>
                    <div class="target-row" style="color: ${isAchieved ? '#22c55e' : '#e67e22'}">
                        <span class="label">${isAchieved ? '✅ Goal Achieved' : 'Target:'}</span>
                        <span class="data-target">${Math.round(m.goal).toLocaleString()}</span><span class="unit">${m.unit}</span>
                    </div>
                </div>
                ${actionButtons}
            </div>
    ...
    // Lógica de la barra de eficiencia
